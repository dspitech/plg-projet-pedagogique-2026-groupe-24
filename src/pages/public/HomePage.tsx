...
interface Stat { label: string; value: number; icon: LucideIcon; hint: string; to: string; gradient: string; }
...
export default function HomePage() {
  const [stats, setStats] = useState<Stat[]>([
    { label: 'Scripts publiés', value: 0, icon: FileCode, hint: 'Voir le catalogue', to: '/nos-scripts', gradient: 'from-blue-500/20 to-cyan-500/10' },
    { label: 'Catégories', value: 0, icon: FolderOpen, hint: 'Explorer par thème', to: '/nos-categories', gradient: 'from-violet-500/20 to-fuchsia-500/10' },
    { label: 'Ressources', value: 0, icon: BookOpen, hint: 'Documents & liens', to: '/nos-ressources', gradient: 'from-emerald-500/20 to-teal-500/10' },
    { label: 'Vues totales', value: 0, icon: Eye, hint: 'Sur tous les scripts', to: '/nos-scripts', gradient: 'from-amber-500/20 to-orange-500/10' },
  ]);
  ...
  useEffect(() => {
    (async () => {
      const [{ count: sc }, { count: cc }, { count: rc }, scr, cats, res, viewsAgg] = await Promise.all([
        ...
        supabase.from('scripts').select('views_count').eq('visibility', 'public'),
      ]);
      const recentScripts = (scr.data ?? []) as ScriptItem[];
      const totalViews = ((viewsAgg.data ?? []) as Array<{ views_count: number | null }>).reduce((a, s) => a + (s.views_count ?? 0), 0);
      setStats([
        { label: 'Scripts publiés', value: sc ?? 0, icon: FileCode, hint: 'Voir le catalogue', to: '/nos-scripts', gradient: 'from-blue-500/20 to-cyan-500/10' },
        { label: 'Catégories', value: cc ?? 0, icon: FolderOpen, hint: 'Explorer par thème', to: '/nos-categories', gradient: 'from-violet-500/20 to-fuchsia-500/10' },
        { label: 'Ressources', value: rc ?? 0, icon: BookOpen, hint: 'Documents & liens', to: '/nos-ressources', gradient: 'from-emerald-500/20 to-teal-500/10' },
        { label: 'Vues totales', value: totalViews, icon: Eye, hint: 'Sur tous les scripts', to: '/nos-scripts', gradient: 'from-amber-500/20 to-orange-500/10' },
      ]);
    ...
    <PublicLayout title="Accueil" description="Scripts Hub Tools — plateforme moderne et sécurisée pour partager et gérer vos scripts Cloud, DevOps et infrastructure.">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container relative mx-auto px-4 lg:px-6 py-20 lg:py-32">
          ...
          {/* Stats — 4 cards cliquables vers les pages concernées */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-16 max-w-5xl mx-auto">
            {stats.map((s, i) => (
              <Link
                key={s.label}
                to={s.to}
                aria-label={`${s.label} : ${s.value.toLocaleString('fr-FR')} — ${s.hint}`}
                className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card/60 backdrop-blur transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/50 hover:shadow-[0_24px_70px_-30px_hsl(var(--primary)/0.55)] focus:outline-none focus-visible:ring-2 focus-visible:ring-primary animate-fade-in"
                style={{ animationDelay: `${i * 80}ms` }}
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${s.gradient} opacity-60 group-hover:opacity-100 transition-opacity`} />
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,hsl(var(--primary)/0.22),transparent_60%)] opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="relative p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-background/70 text-primary border border-border/60 backdrop-blur shadow-sm transition-transform group-hover:scale-110">
                  ...
                  <div className="text-4xl font-extrabold tracking-tight tabular-nums leading-none">
