...
export default function ContactPublicPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    const { data, error } = await supabase.functions.invoke('public-contact-submit', {
      body: {
    ...
    if (error || errCode) {
      if (errCode === 'rate_limited') toast.error("Trop de messages envoyés. Réessayez plus tard.");
      else if (errCode === 'invalid_input') toast.error("Données invalides. Vérifiez le formulaire.");
      else toast.error("Erreur lors de l'envoi du message.");
      return;
    }
